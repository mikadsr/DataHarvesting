...
> **NOTE**: This step loops through 1117 links so please expect it to take a while. Further down this .qmd you will get theoption of running everything either on the complete scraped data or the sampled data. To run on the sampled data just turn eval = TRUE on the {r} at the top of the chunk.
...
filtered_rows <- side_effects_sample |> 
...
#filtered_rows2 <- side_effects_sample |> 
...
```{r SAMPLE match the ATC codes to indications}
...
temp_matches <- sampled_side_effects |> 
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("atc_codes", "Select ATC Code:", choices = unique(sampled_side_effects$atc_codes)),
      selectInput("selected_drug", "Select Drug:", choices = c(NULL, unique(sampled_side_effects$drug_name))),
      width = 3
    ),
    mainPanel(
      tabsetPanel(
...
server <- function(input, output, session) {
  ...
  filtered_ocde_data <- reactive({
    ocde |>
      filter(varcode == input$atc_codes)
  })
...
For our first visualization, we created an interactive map. Our process began with data cleaning, which involved merging the OCDE dataset with the indications and side_effects tables. Due to the complexity of the data for direct mapping, we made several decisions to streamline the information:
...
-   From the filtered side effects, we randomly chose 5 to include in the visualization. This approach provided a representative sample of side effects associated with each drug.
...
max_indications_drug <- indications_sample |> 
  group_by(drug_name) |> 
  summarize(total_indications = n()) |> 
  inner_join(sampled_side_effects, by = "drug_name") |> 
  group_by(atc_codes) |> 
...
map <- ocde_max |> 
  ...
  group_by(country) |> 
  mutate(drug_name = if (any(atc_codes == "C") & n_distinct(drug_name) > 1) sample(drug_name, 1) else drug_name) |> 
  ungroup() |> 
